// ==UserScript==
// @name         trwiki-football-stats-helper
  ...
  `;
	...
	function ensureQuickAccessLink() {
		...
		quickAccessLink = window.mw.util.addPortletLink(
			...
			'Duzenleme ekraninda futbolcu istatistik aracini ac'
		);
	...
	function ensureUi() {
		...
		if ( isMonobookSkin() && window.mw && window.mw.util && window.mw.util.addPortletLink ) {
			launchButton = window.mw.util.addPortletLink(
				...
				'Futbolcu istatistik aracini ac'
			);
		...
		if ( !launchButton ) {
			...
			launchButton.textContent = 'MatchStats Editor';
		...
		backdrop.innerHTML = `
      <div class="tfsh-modal" role="dialog" aria-modal="true" aria-label="MatchStats Editor">
        <div class="tfsh-head">
          <div class="tfsh-title">MatchStats Editor</div>
